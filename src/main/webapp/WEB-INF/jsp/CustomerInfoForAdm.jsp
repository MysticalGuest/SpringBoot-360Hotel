   		...
   		<script>
   			...
   			function doSearch(){
   				...
   				$.ajax({
					...
					// url:'../DistinguishedQuality360Hotel/administrator/CustomerInfoForAdm',
   					type: 'post',
   					...
   					error:function(xhr,type,errorThrown){
						$.messager.alert('提示','搜索失败！','info',
							function(){location.href='../DistinguishedQuality360Hotel/administrator/CustomerInfoForAdm'});
   		...
   		<script>
			...
			function deleteChecked(){
				...
				$.each(checkedItems, function (index, item) {
					customers.push(item.inTime);
				});
				//将customers数组转为字符串传到后台
				var strInTime=customers.join(",");
				if(customers==""){
					$.messager.alert('提示','您未选中任何顾客！','warning') 
				}
				else{
					$.ajax({
						url: '../DistinguishedQuality360Hotel/administrator/deleteChecked',
						data:{'strInTime':strInTime},
						type:'post',//HTTP请求类型
						dataType:'json',//服务器返回json格式数据
						// timeout:10000,//超时时间设置为10秒；
						success:function(data){
   	...
   	<div data-options="region:'center',border:true,title:'顾客信息'" style="background:#fff;" >
    		<div style="width:100%;height:100%;float: right;padding: 5px;">
    			<table id="dg" class="easyui-datagrid" style="width:100%;height:100%"
    				fitColumns="true" idField="inTime" data-options="
						rownumbers:true,
						autoRowHeight:true,
						pagination:true,
						pageSize:10">
